...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // If LinkedList is empty, either pushFront or pushBack is OK.
  // Arbitrarily choose pushFront for empty case
  // and combine with case where newData is smallest of all data.
  // Node with newData goes to front of LinkedList
  if ((!head_) or (LinkedList<T>::front() > newData)) {
    // std::cout << "LinkedList before insertion:" << *this << std::endl;

    LinkedList<T>::pushFront(newData);

    // std::cout << "inserted at head" << newData << std::endl;
    // std::cout << "LinkedList after insertion:" << *this << std::endl;
  }

  // newData is largest of all data.
  // Node with newData goes to back of LinkedList
  else if (LinkedList<T>::back() < newData) {
    // std::cout << "LinkedList before insertion:" << *this << std::endl;

    LinkedList<T>::pushBack(newData);

    // std::cout << "inserted at tail" << newData << std::endl;
    // std::cout << "LinkedList after insertion:" << *this << std::endl;
  }

  // newData is somewhere in the middle of the LinkedList,
  // with at least one Node before and at least one Node after.
  else {
    // Both pushFront and pushBack create new Node on heap already.
    // Inly this if-else case needs Node constructor

    // std::cout << "LinkedList:" << *this << std::endl;

    // std::cout << "inserting at middle" << newData << std::endl;

    Node* newNode = new Node(newData);

    // std::cout << "newData:" << newData << std::endl;
    // std::cout << "newNode contents:" << newNode->data << std::endl;

    Node* prevNode = head_;
    Node* nextNode = head_->next;

    // prevNode and nextNode are adjacent Nodes
    // until newNode is inserted in between prevNode and nextNode.
    // travel forward through LinkedList until data at nextNode > newData
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // left "working copy" and right "working copy" are empty.
  // return empty merged LinkedList as initialized
  if ((!left.head_) and (!right.head_)) {
    return merged;
  }

  /*
  // left "working copy" is empty.
  // make merged LinkedList a copy of right "working copy"
  else if (!left.head_) {

    // pointer to const Node to iterate through right "working copy"
    const Node* ptrRight = right.head_;

    // iterate through right "working copy"
    while (ptrRight) {
      // new Node on heap to add to merged LinkedList
      Node* newNode = new Node(ptrRight->data);

      Node* oldTail = merged.tail_;
  ...
  // right "working copy" is empty.
  // make merged LinkedList a copy of left "working copy"
  else if (!left.head_) {

    // pointer to const Node to iterate through left "working copy"
    const Node* ptrLeft = left.head_;

    // iterate through left "working copy"
    while (ptrLeft) {
      // new Node on heap to add to merged LinkedList
      Node* newNode = new Node(ptrLeft->data);
  ...
  */
  ...
  /* remove this middle case where only one "working copy" has at least one node,
     because it can be combined with latter part of last case where
     both left and right "working copy" have at least one node
  // combine into a single case where one and only one is true:
  // right "working copy" is empty and left "working copy" has at least one Node, or
  // left "working copy" is empty and right "working copy" has at least one Node.
  else if ((!left.head_) or (!right.head_)) {
    ...
    // if left "working copy" is empty
    // then initialize Node* to head of right "working copy"
    if (!left.head_) {
      // pointer to const Node to iterate through right "working copy"
      // const Node* ptrLeftOrRight = right.head_;
      ptrLeftOrRight = right.head_;
    }
    // otherwise right "working copy" is empty
    // so initialize Node* to head of left "working copy"
    else {
      // pointer to const Node to iterate through left "working copy"
      // const Node* ptrLeftOrRight = left.head_;
      ptrLeftOrRight = left.head_;
    }

    // iterate through "working copy"
    while (ptrLeftOrRight) {
      // new Node on heap to add to merged LinkedList
      Node* newNode = new Node(ptrLeftOrRight->data);

      // insert newNode as the new tail of merged LinkedList
      if (!merged.tail_) {
      ...
      // advance pointer
      ptrLeftOrRight = ptrLeftOrRight->next;
    }
    return merged;
  }
  */
  ...
  // (b) only one of left "working copy" and right "working copy" have at least one Node
    ...
    // only while a Node remains to compare
    // in both left "working copy" and right "working copy",
    // compare both left "working copy" and
    // right "working copy"
    while ((ptrLeft) and (ptrRight)) {
      // std::cout << "one Node remains in left and right" << std::endl;
      // std::cout << "left:" << left << std::endl;
      // std::cout << "right:" << right << std::endl;

      // alias to Node pointer needed so that left or right Node pointer advances
      // lambda function assigns pointer for "working copy" with lesser data
      Node*& ptrLeftOrRight = [&ptrLeft, &ptrRight]() -> Node*& {
      ...
      // note () needed to invoke lambda function
      }();

      // ptrLeftOrRight allows code reuse rather than copying the same code
      // for advancing a single Node in left "working copy" and advancing a
      // single Node in right "working copy"
/*
      // choose whether to advance ptrLeft or ptrRight
      // depending on which data is bigger in dereferenced Node
      // of ptrLeft and ptrRight
      if (ptrLeft->data <= ptrRight->data) {
        // since data in dereferenced Node of ptrLeft is <=
        // data in dereferenced Node of ptrRight,
        // work on ptrLeft
        Node*& ptrLeftOrRight = ptrLeft;
        std::cout << "work on ptrLeft" << std::endl;
        std::cout << ptrLeftOrRight
      }
      else {
        // since data in dereferenced Node of ptrLeft is >
        // data in dereferenced Node of ptrRight,
        // work on ptrRight
        Node*& ptrLeftOrRight = ptrRight;
        std::cout << "work on ptrRight" << std::endl;
      }
*/
      // new Node on heap to add to merged LinkedList
      Node* newNode = new Node(ptrLeftOrRight->data);
